#1.patient_GSFM
exp <- rio::import("../2.data transformation/project/Sample_output/BRCA/TCGA_BRCA_patient_GSFM.csv")
rownames(exp) <- exp$V1
exp <- exp[,-1]
...
#1.3 Patient data value range (-1,1)
...
#2.drug_GSFM
drug <- rio::import("../2.data transformation/project/Sample_output/BRCA/LINCS_MCF7_drug_GSFM.csv")
rownames(drug) <- drug$V1
drug <- drug[,-1]
drug <- as.data.frame(t(drug))

#2.1 Drug data value range (-1,1)
data_drug <- list()
count=0
...
#5.RS——IC50
data <- rio::import("GSFM/BRCA_score_GSFM_all.csv")
library(dplyr)
data <- data[,-1]
data$id <- toupper(data$id)

#5.2 Convert drug names and get intersection
...
#5.3 Import IC50 from CHEMBL database
...
bar_plot =  ggplot(drug_activity_RS, aes(activity, RS,colour  = activity  )) + geom_boxplot() +  coord_flip()+
  ...
  annotate("text", label = paste("P=", format(efficacy_test$p.value, digit=3, scientific=T), sep=""), 
           x = 1.5, y = -0.23, size = 6, colour = "black") + theme(legend.position ="none")+
